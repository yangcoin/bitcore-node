...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
    ...
    //var queryMempool = false; //_.isUndefined(options.queryMempool) ? true : options.queryMempool;
    var addresses = self._normalizeAddressArg(addressArg);
    var cacheKey = addresses.join('');
    ...
    function updateWithMempool(confirmedUtxos, mempoolDeltas) {
        ...
        if (isSpentOutputs) {
            var rt = utxos.filter(function(utxo) {
    ...
    if (tx) {
        ...
        self._tryAllClients(function(client, done) {
            client.getRawTransaction(txid, 1, function(err, response) {
                ...
                if (result.vin[0] && result.vin[0].coinonline) {
                    tx.coinonline = true;
                }else if (result.vin[0] && result.vin[0].coinbase) {
                    tx.coinbase = true;
                }else if(result.vin.length> 0 && result.vout.length >=2 && result.vout[0].value==0 && result.vout[0].spentTxId == undefined ){
                    tx.coinstake = true;
                }
